...
(function() {
  var Galaxy, KEY_DOWN, KEY_LEFT, KEY_RIGHT, KEY_SPACE, KEY_UP, Planet, Player, galaxy, h, key_dir, planets, player, rotation, w;
  ...
  Player = (function() {
    ...
    Player.prototype.jump_velocity = 0;

    Player.prototype.max_jump = 40;

    Player.prototype.min_jump = 5;
    ...
    Player.prototype.accel = function(angle, force, cap) {
      ...
      if (cap && Math.sqrt((Math.pow(this.velocityX, 2)) + (Math.pow(this.velocityY, 2)) > this.max_speed)) {
        angle = Math.PI / 2 - Math.atan2(this.velocityX, this.velocityY);
        this.velocityX = (Math.cos(angle)) * this.max_speed;
        return this.velocityY = (Math.sin(angle)) * this.max_speed;
      }
    };
    ...
    Player.prototype.findNearestPlanet = function(planets, log) {
      ...
      _fn = function(planet) {
        ...
        if (dist < nearestDistance) {
          ...
          if (log) {
            return console.log("Nope! " + _this.distanceTo(planet) + " > " + nearestDistance);
          }
        }
      };
      for (_i = 0, _len = planets.length; _i < _len; _i++) {
    ...
    Player.prototype.startJumping = function() {
      var cb,
        _this = this;
      this.jumping = true;
      this.jump_velocity = this.min_jump;
      cb = function() {
        console.log("cb: " + _this.jump_velocity);
        _this.jump_velocity = _this.jump_velocity + 1 < _this.max_jump ? _this.jump_velocity + 1 : _this.max_jump;
        if (_this.jumping) {
    ...
    Player.prototype.jump = function() {
      console.log("Jump! " + this.jump_velocity);
      this.accel(window.rotation - Math.PI, this.jump_velocity);
      this.jumping = false;
      return this.jump_velocity = this.min_jump;
    };
    ...
    Player.prototype.draw = function(s, g) {
      ...
      this.s.noStroke();
      this.s.fill(255, 0, 0);
      return this.s.rect(x - this.width - 2, y - this.height, this.width, this.height);
    };
  ...
  sketch(function() {
    ...
    this.setup = function() {
      _this.size(w, h);
      _this.background(255);
      _this.noFill();
      return _this.frameRate(30);
    };
